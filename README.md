...
Capturing the key presented at `127.0.0.1:2222` is an explicit trust-on-first-use step for this local fixture. The application rejects any other key. `make integration` checks real SFTP read, write, overwrite, collision, transfers, and error cases. Start the API with `make run`; it serves `/openapi.json` and `/docs`. Connection creation, inspection, local/SFTP listing, and synchronous transfers are implemented. Preview follows in a subsequent commit.
