...
2026-09-24 gates: `make check` with a writable temporary uv cache passed Ruff and 17 local/API tests; `make integration` passed 7 real Docker SFTP tests. The scaffold's live Uvicorn process returned HTTP 200 at `/openapi.json`; transfer and preview routes have been exercised through FastAPI TestClient but not yet over live HTTP.
