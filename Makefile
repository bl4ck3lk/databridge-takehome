...
# Targets that read .env: the service settings, and the Docker SFTP fixture's port and trust file.
...
run:
	...
	@uv run --env-file .env python -m databridge serve

logs:
	$(require_env)
	@log="$$(uv run --env-file .env python -m databridge request-log-path)" && tail -n 50 -F "$$log"
